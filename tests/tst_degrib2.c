...
#define FTP_FILE_2 "gdas.t12z.pgrb2.1p00.anl.grib2"
#define FTP_DEGRIB2_FILE_2 "gdas.t12z.pgrb2.1p00.anl.degrib2"
#define REF_FTP_DEGRIB2_FILE_2 "ref_gdas.t12z.pgrb2.1p00.anl.degrib2"
#define FTP_INDEX_FILE_2 "gdas.t12z.pgrb2.1p00.anl.grb2index"
#define FTP_NUM_MSG_2 2
...
#define NUM_TESTS 2
	...
	int ret;
        ...
        for (t = 0; t < NUM_TESTS; t++)
        {
            ...
            {
                printf("\ttesting degrib2 on file %s downloaded via FTP using index...", FTP_FILE);
                if ((ret = g2c_read_index(FTP_FILE_2, FTP_INDEX_FILE_2, 0, &g2cid)))
                    return ret;
            }
            else
            {
                printf("\ttesting degrib2 on file %s downloaded via FTP without using index...", FTP_FILE_2);
                if ((ret = g2c_open(FTP_FILE_2, 0, &g2cid)))
                    return ret;

                if ((ret = g2c_write_index(g2cid, 0, FTP_INDEX_FILE_2)))
                    return ret;
            }
            ...
            for (m = 0; m < num_msg; m++)
            {
                if ((ret = g2c_inq_msg(g2cid, m, &discipline, &num_fields, &num_local,
                                       &center, &subcenter, &master_version, &local_version)))
                    return ret;
                /* printf("discipline %d num_fields %d num_local %d center %d subcenter %d master_version %d local_version %d\n", */
                /*        discipline, num_fields, num_local, center, subcenter, master_version, local_version); */
                if (center != 7 || subcenter != 0 || master_version != 2 || local_version != 1)
                    return G2C_ERROR;
                if (num_fields != 1 || num_local)
                    return G2C_ERROR;
            ...
            /* Check some stuff about the last message. */
            if ((ret = g2c_inq_msg(g2cid, 1, &discipline, &num_fields, &num_local,
                                   &center, &subcenter, &master_version, &local_version)))
                return ret;
            if (center != 7 || subcenter != 0 || master_version != 2 || local_version != 1)
                return G2C_ERROR;
            if (num_local || num_fields != 1 || discipline != 0)
                return G2C_ERROR;
            ...
            /* Inquire about the date/time. */
            {
                ...
                if (sig_ref_time != 0 || year != 2022 || month != 11 || day != 6 ||
                    hour != 12 || minute != 0 || second != 0)
                    return G2C_ERROR;
            }
            
            /* Output a degrib2 file. */
            if ((ret = g2c_degrib2(g2cid, FTP_DEGRIB2_FILE_2)))
                return ret;
            ...
            /* Compare the degrib2 output to our reference file. */
            if ((ret = compare_files2(FTP_DEGRIB2_FILE_2, REF_FTP_DEGRIB2_FILE_2)))
                return ret;
