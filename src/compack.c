        ...
        unsigned char *cpack, g2int *lcpack)
{
    ...
    /*  If max and min values are not equal, pack up field. If they
     *  are equal, we have a constant field, and the reference value
     *  (rmin) is the value for each point in the field and set nbits
     *  to 0. */
    if (rmin != rmax) {
        iofst=0;
        ...
        if (idrsnum == 3) {        /* spatial differences */
            ...
            /*   find num of bits need to store minsd and add 1 extra
             *   bit to indicate sign */
            ...
            /*  Store extra spatial differencing info into the packed data section. */
            if (nbitsd != 0) {
        ...
        if ( simple_alg == 1 ) {
            /*  set group length to 10; calculate number of groups and
         ...
         *  needed to pack each groups length value, then pack up
         *  group length values */
