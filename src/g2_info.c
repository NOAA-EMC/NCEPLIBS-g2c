        ...
        g2int *numfields, g2int *numlocal)
{
    ...
    ret = g2c_info(cgrib, &discipline, &message_len, listsec1_int, &numfields_int,
                   &numlocal_int, NULL, NULL, NULL);

    /* Translate int types back to g2int. */
    *numfields = numfields_int;
    *numlocal = numlocal_int;
    listsec0[0] = discipline;
    listsec0[1] = 2;
    listsec0[2] = message_len;
 ...
 *
 * @param cgrib Pointer to a buffer containing the GRIB2 message.
 * @param discipline Pointer that gets the discipline value from
 * ([Code Table 0.0]
 * (https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_table0-0.shtml))
 * of the GRIB2 message. Ignored if NULL.
 * @param message_len Pointer that gets the total length of the GRIB2
 * message, in bytes. Ignored if NULL.
 * @param listsec1 Pointer to an array that gets the information read
 * from GRIB Identification Section 1. Must be allocated with >= 13
 ...
 */
int
g2c_info(unsigned char *cgrib, int *discipline, size_t *message_len, int *listsec1,
         ...
         size_t *section_offset)
{
    ...
    if (discipline)
        *discipline = my_listsec0[0];
    if (message_len)
        *message_len = my_listsec0[2];
    if (listsec1)
	for (i = 0; i < G2C_SECTION1_LEN; i++)
 ...
 * performed to see if the message is a valid GRIB2 message.
 *
 * @param g2cid File ID, returned from g2c_open() or g2c_create().
 * @param skip_bytes Number of bytes to skip in file to get to message.
 * @param discipline Pointer that gets the discipline value from
 * ([Code Table 0.0]
 * (https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_table0-0.shtml))
 * of the GRIB2 message. Ignored if NULL.
 ...
 */
int
g2c_msg_info(int g2cid, size_t skip_bytes, int *discipline, size_t *message_len, int *listsec1,
             int *numfields, int *numlocal, int *numsection, int *section_number,
             size_t *section_offset)
{
    /* int mapsec1[G2C_SECTION1_LEN] = {2, 2, 1, 1, 1, 2, 1, 1, 1, 1, 1, 1, 1}; */
    /* int ipos, nbits, lensec; */
    /* int offset, lengrib, lensec0, lensec1, secnum; */
    /* int my_listsec0[G2C_SECTION0_LEN], my_listsec1[G2C_SECTION1_LEN]; */
    /* int my_numlocal = 0, my_numfields = 0; */
    /* int ns = 0; /\* number of sections *\/ */
    /* int my_section_number[G2C_MAX_NUM_SECTIONS]; */
    ...
    /* LOG((2, "g2c_msg_info offset %ld", skip_bytes)); */
